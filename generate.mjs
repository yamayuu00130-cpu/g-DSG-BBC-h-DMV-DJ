...
const games=[
  {
    ...
    publisher:"Mojang / Microsoft",emoji:"⛏️",sourceType:"タイトル公式",
    officialUrl:"https://www.minecraft.net/en-us/usage-guidelines",checked:"2026-08-08",
    overall:"条件あり",
  ...
  {
    slug:"splatoon-3",title:"Splatoon 3",
    aliases:["スプラ3","スプラトゥーン3","Splatoon3"],
    publisher:"Nintendo",emoji:"🎨",sourceType:"メーカー共通",
    officialUrl:"https://www.nintendo.co.jp/networkservice_guideline/ja/index.html",checked:"2026-08-08",
    overall:"条件あり",
  ...
  {
    slug:"silent-hill-f",title:"SILENT HILL f",
    aliases:["サイレントヒルf","サイレントヒルエフ","SHf"],
    publisher:"KONAMI",emoji:"🌫️",sourceType:"タイトル公式",
    officialUrl:"https://www.konami.com/games/silenthill/f/guideline/jp/ja/",checked:"2026-08-08",
    overall:"条件あり",
  ...
  },
  {
    ...
    publisher:"Bandai Namco Entertainment",emoji:"🗡️",sourceType:"メーカー共通",
    officialUrl:"https://www.bandainamcoent.co.jp/info/videopolicy/",checked:"2026-08-08",
    overall:"条件あり",
...
const norm=s=>(s||'').normalize('NFKC').toLowerCase().replace(/[ぁ-ゖ]/g,c=>String.fromCharCode(c.charCodeAt(0)+0x60)).replace(/[ーｰ\-‐‑‒–—―_\s・･:：'"“”‘’!！?？.,，．/／\\()\[\]{}【】「」『』]/g,'');
function score(q,g){const nq=norm(q);let best=0;for(const v of [g.title,g.publisher,...g.aliases].map(norm)){if(v===nq)best=Math.max(best,100);else if(v.startsWith(nq))best=Math.max(best,85);else if(v.includes(nq))best=Math.max(best,70);else if(nq.includes(v)&&v.length>=3)best=Math.max(best,55)}return best}
let searchResults=[];
function openSuggestion(slug){const b=document.getElementById('suggestions');if(b)b.style.display='none';location.href='/games/'+slug+'/'}
function suggest(){const input=document.getElementById('q'),b=document.getElementById('suggestions');if(!input||!b)return;const q=input.value;if(!q.trim()){b.style.display='none';b.innerHTML='';searchResults=[];return}searchResults=games.map(g=>[score(q,g),g]).filter(x=>x[0]>0).sort((a,b)=>b[0]-a[0]).slice(0,6);b.innerHTML=searchResults.map(x=>'<button type="button" class="suggestion" data-slug="'+x[1].slug+'"><b>'+x[1].title+'</b><span class="small">'+x[1].publisher+' / '+x[1].aliases.slice(0,3).join('・')+'</span></button>').join('');b.style.display=searchResults.length?'block':'none';b.querySelectorAll('.suggestion').forEach(btn=>btn.addEventListener('pointerdown',e=>{e.preventDefault();openSuggestion(btn.dataset.slug)}))}
function searchSubmit(e){e.preventDefault();const q=document.getElementById('q').value;const f=games.map(g=>[score(q,g),g]).filter(x=>x[0]>0).sort((a,b)=>b[0]-a[0]);if(f[0])openSuggestion(f[0][1].slug);else location.href='/games/'}
document.addEventListener('pointerdown',e=>{const b=document.getElementById('suggestions');if(b&&e.target instanceof Element&&!e.target.closest('.search-wrap'))b.style.display='none'});
`;
...
const homeCards=games.map(g=>`<a class="card game-card" href="/games/${g.slug}/"><span class="ico">${g.emoji}</span><span><b>${esc(g.title)}</b><span class="small">${esc(g.publisher)}</span></span><span class="pill">⚠️ ${esc(g.overall)}</span></a>`).join("");
write("index.html",shell("配信OK？｜ゲーム配信ガイドライン検索",`<main class="container"><section class="hero"><div><h1>このゲーム、<br><span>配信して大丈夫？</span></h1><p>公式ガイドラインを配信者目線で整理。収益化・法人利用・配信条件を公式ソース付きで確認できます。</p><div class="search-wrap"><form class="search" onsubmit="searchSubmit(event)"><input id="q" oninput="suggest()" placeholder="例：マイクラ / スプラ3 / エルデン"><button>検索</button></form><div id="suggestions" class="suggestions"></div></div></div><div class="hero-art"><div>🎙️</div></div></section><section class="section"><h2>人気のガイドライン</h2><p class="lead">まず答えを出して、その後に必要な解説や収益コンテンツへ進む構成。</p><div class="grid">${homeCards}</div></section></main>`));

const list=games.map(g=>`<a class="card game-card" href="/games/${g.slug}/"><span class="ico">${g.emoji}</span><span><b>${esc(g.title)}</b><span class="small">${esc(g.publisher)}</span></span><span>→</span></a>`).join("");
write("games/index.html",shell("ゲーム配信ガイドライン一覧｜配信OK？",`<main class="container section"><h1>ゲーム配信ガイドライン一覧</h1><p class="lead">登録済みタイトル</p><div class="grid">${list}</div></main>`));

for(const g of games){
  const rules=g.rules.map(([n,st,lab,note])=>`<div class="rule"><b>${esc(n)}</b><span class="status ${st}">${st==="ok"?"✅":st==="warn"?"⚠️":"❓"} ${esc(lab)}</span><span class="note">${esc(note)}</span></div>`).join("");
  write(`games/${g.slug}/index.html`,shell(`${g.title}の配信ガイドライン｜配信OK？`,`<main class="container result"><section class="card" style="padding:0;overflow:hidden"><div class="game-head"><div class="cover">${g.emoji}</div><div><h1 style="margin:0 0 7px">${esc(g.title)}</h1><span class="small">${esc(g.publisher)} / ${esc(g.sourceType)}<br>最終確認 ${esc(g.checked)}</span></div><div class="overall">⚠️<br>${esc(g.overall)}</div></div><div class="summary">${esc(g.summary)}　<a href="${g.officialUrl}" target="_blank" rel="noopener noreferrer" style="color:var(--g2);font-weight:850">公式ガイドライン ↗</a></div><div class="rules">${rules}</div><div class="notice">回答を広告で隠さない設計です。配信前には公式情報を最終確認してください。</div></section><div class="ad-slot">広告枠（本番でAdSense接続）</div><section class="revenue-box"><h3>${esc(g.title)}を配信する準備もチェック</h3><p class="lead">権利情報を確認したら、配信設定・PC・マイク・キャプチャー環境なども確認できます。</p><a class="btn" href="/guides/streaming-setup/">配信環境の作り方</a></section><div class="ad-slot">広告枠（本番でAdSense接続）</div></main>`));
}
const guides=[["streaming-setup","ゲーム配信環境の作り方"],["obs-basics","OBSの基本設定"],["vtuber-stream-checklist","VTuber配信前チェックリスト"]];
write("guides/index.html",shell("配信ノウハウ｜配信OK？",`<main class="container section"><h1>配信ノウハウ</h1><p class="lead">権利チェッカーから自然につながる収益コンテンツ枠。</p><div class="guide-grid">${guides.map(([s,t])=>`<a class="guide-card" href="/guides/${s}/"><h3>${t}</h3><p>実用情報を優先して、広告・アフィリエイト導線を自然に配置します。</p></a>`).join("")}</div></main>`));
for(const [s,t] of guides)write(`guides/${s}/index.html`,shell(`${t}｜配信OK？`,`<main class="container section"><h1>${t}</h1><p class="lead">収益化記事の土台です。広告・アフィリエイト導入時も内容の有用性を優先します。</p></main>`));
for(const [p,t,txt] of [["changes","ガイドライン更新履歴","Workersが変更候補を検出し、人間確認後に公開する想定です。"],["terms","利用規約・免責事項","本サービスは配信許可や適法性を保証しません。"],["privacy","プライバシーポリシー","静的閲覧を基本とし、必要以上の個人情報を収集しない方針です。"],["corrections","掲載情報の修正について","公式ソースを確認して修正できる運用を行います。"]])write(`${p}/index.html`,shell(`${t}｜配信OK？`,`<main class="container section"><h1>${t}</h1><p class="lead">${txt}</p></main>`));
