...
.hero p,.lead{color:var(--muted);line-height:1.8}
...
.suggestions{
  ...
  border:1px solid var(--line);
  border-radius:15px;
  box-shadow:0 18px 50px rgba(20,48,31,.12);
  overflow-y:auto;
...
.suggestion+.suggestion{margin-top:3px}
.suggestion:active,.suggestion:hover{background:#edf9f2}
...
.suggestion .small{line-height:1.45}
...
.cover{
  ...
  height:100px;
  border-radius:17px;
  background:linear-gradient(145deg,#159a5a,#263c30);
  display:grid;
  place-items:center;
  color:#fff;
  font-size:42px;
...
.rules{padding:8px 23px}

.rule{
  display:grid;
  grid-template-columns:1.05fr 130px 1.7fr;
  gap:14px;
  padding:15px 0;
  border-bottom:1px solid var(--line)
}
...
.status.warn{color:#b56d13}
.status.unknown{color:#7b8580}
...
@media(max-width:800px){
  ...
  .grid,.guide-grid{grid-template-columns:1fr}
...
function safeImageUrl(game){
  if(!game || !game.image_url)return null;

  // candidate / approved の公式ページ取得画像を表示
  if(
    game.image_status!=="candidate" &&
    game.image_status!=="approved"
  ){
    return null;
  }

  try{
    const u=new URL(game.image_url);
...
function setImage(slot,game){
  const imageUrl=safeImageUrl(game);
...
async function hydrateGameImages(){
  try{
    ...
    document
      .querySelectorAll("[data-game-image]")
      .forEach(slot=>{
        const name=slot.getAttribute("data-game-image");

        const game=dbGames.find(g=>g.name===name);
...
function suggestionImage(game){
  const url=safeImageUrl(game);
  const emoji=staticEmoji[game.name]||"🎮";

  if(!url){
    return '<span class="suggestion-image">'+emoji+'</span>';
  }

  return (
    '<span class="suggestion-image">'+
      '<img src="'+htmlEscape(url)+'" '+
      'alt="" loading="lazy" '+
...
async function runSuggest(){
  ...
  try{
    ...
    if(!searchResults.length){
      box.innerHTML=
        '<div class="suggestion" style="cursor:default;grid-template-columns:1fr">'+
        '<span>'+
          '<b>該当するゲームがありません</b>'+
...
<header class="header">
<a class="logo" href="/">配信<span>OK</span>？</a>
<nav>
<a href="/games/">ゲーム</a>
<a href="/guides/">配信ノウハウ</a>
...
<a href="/corrections/">情報修正</a>
</nav>
<div>本サービスは配信許可や法的判断を保証するものではありません。</div>
</div>
</footer>
...
<a class="card game-card" href="/games/${g.slug}/">
  <span class="ico" data-game-image="${esc(g.title)}">${g.emoji}</span>
  <span>
    <b>${esc(g.title)}</b>
    <span class="small">${esc(g.publisher)}</span>
  </span>
  <span class="pill">⚠️ ${esc(g.overall)}</span>
</a>
`).join("");
...
<p>
公式ガイドラインを配信者目線で整理。
収益化・法人利用・配信条件を公式ソース付きで確認できます。
</p>

<div class="search-wrap">

<form class="search" onsubmit="searchSubmit(event)">
<input
  id="q"
  oninput="suggest()"
  placeholder="例：マイクラ / スプラ3 / エルデン"
>
<button>検索</button>
</form>
...
<p class="lead">
まず答えを出して、その後に必要な解説や収益コンテンツへ進む構成。
</p>
...
<a
  ...
  style="color:var(--g2);font-weight:850"
>
公式ガイドライン ↗
</a>
...
<h3>
${esc(g.title)}を配信する準備もチェック
</h3>

<p class="lead">
権利情報を確認したら、
配信設定・PC・マイク・キャプチャー環境なども確認できます。
</p>

<a class="btn" href="/guides/streaming-setup/">
配信環境の作り方
</a>
...
<p class="lead">
権利チェッカーから自然につながる収益コンテンツ枠。
</p>

<div class="guide-grid">

${guides.map(([s,t])=>`
<a class="guide-card" href="/guides/${s}/">
<h3>${t}</h3>
<p>
実用情報を優先して、
広告・アフィリエイト導線を自然に配置します。
</p>
</a>
`).join("")}
...
for(const [p,t,txt] of [
  [
    "changes",
    "ガイドライン更新履歴",
    "Workersが変更候補を検出し、人間確認後に公開する想定です。"
  ],
  [
    "terms",
    "利用規約・免責事項",
    "本サービスは配信許可や適法性を保証しません。"
  ],
  [
    "privacy",
    "プライバシーポリシー",
    "静的閲覧を基本とし、必要以上の個人情報を収集しない方針です。"
  ],
  [
    "corrections",
    "掲載情報の修正について",
    "公式ソースを確認して修正できる運用を行います。"
  ]
]){
